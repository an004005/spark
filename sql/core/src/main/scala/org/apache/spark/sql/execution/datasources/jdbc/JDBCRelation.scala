 ...
 *
 * DI Custom, Long, string type JDBCPartitioningInfo case class
 */
...
private[sql] object JDBCRelation extends Logging {
  ...
  def columnPartition(
    ...
    partitioning match {
      ...
      case infoString: JDBCPartitioningInfoString =>
        getPartitionWhereClause(infoString, jdbcOptions)
  ...
  def getPartitionWhereClause(
    ...
    val strides: Array[Long] = if (jdbcOptions.optimizePartition) {
      JdbcUtils.getSampleRowsAndPartition[Long](
        partitioning.column, jdbcOptions, numPartitions, lowerBound,
    ...
    // Here we get a little roundoff, but that's (hopefully) OK.
      ...
      (0L until numPartitions).map(_ => stride).toArray
    }
   ...
   */
  def getPartitionWhereClause(
      partitioning: JDBCPartitioningInfoString,
      jdbcOptions: JDBCOptions): Array[Partition] = {
    ...
    val strides: Array[BigDecimal] = if (jdbcOptions.optimizePartition) {
      JdbcUtils.getSampleRowsAndPartition[BigDecimal](
        partitioning.column, jdbcOptions, numPartitions, lowerBound,
        (rs: ResultSet) => stringToBigDecimal(rs.getString(1))
      ).toArray
    } else {
      val stride: BigDecimal = tryDivide(upperBound - lowerBound, numPartitions)
      (0 until numPartitions).map(_ => stride).toArray
    }
  ...
  def autoColumnPartition(
    ...
    val partitionColumn = dialect.supportedPseudoColumn()
      .orElse(JdbcUtils.getHighCardinalityColumnWithPrimaryKey(conn, jdbcOptions))
      .getOrElse({
